...
## 链路追踪
1. OpenTracing 规范， 为解决不同供应商的分布式追踪系统 API 互不兼容而提供的标准框架， 可以认为是一个接入层
   - 功能上： 提供一系列与供应商无关的 API
   - 系统上： 让开发人员便捷地对接(新增或替换)追踪系统
   - 语言上： 跨语言，通过接口的设计概念去封装一系列 API 的相关功能
   - 常见术语
     - Trace 代表了一个事务或流程在(分布式)系统中的执行过程
     - Span 代表一个事务中的每个工作单元，通常多个 Span 组成一个完整的 Trace
     - SpanContext 一个事务的相关跟踪信息，不同 Span 根据 OpenTracing 规范封装不同的信息
2. 通过 docker 启动 Jaeger -- 一个开源的分布式链路追踪系统
3. 新建中间件，让每次接口调用都被记录到 Jaeger 中，并将 trace span 等信息加入上下文环境的元数据中
4. 理论上这里应该在增加SQL 的追踪，但这里是引用的 私有库?(书的作者的库，怕出现安全问题便不再继续)
